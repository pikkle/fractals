...
public class FlamePPMMaker {
	public static void main(String[] args) throws FileNotFoundException {
		List<Color> listColor = Arrays.asList(new Color[]{Color.RED, Color.GREEN, Color.BLUE});
		...
		FlameAccumulator flameaccuShark = flameShark.compute(new Rectangle(new Point(
		...
		for (int i = 0; i < h; i++) {
			for (int j = 0; j < w; j++) {
				Color colorPrint = flameaccuShark.color(paletteInterpol, Color.BLACK, i, j);
				ps1.print(colorPrint.red()+ " "+ colorPrint.green()+" "+ colorPrint.blue()+" ");
			}
			ps1.print("\n");
		}
		ps1.flush();
		...
		FlameAccumulator flameaccuTurb = flameTurb.compute(new Rectangle(new Point(
				0.1,0.1), 3,3), 500 , 500, 50);
		
		PrintStream ps2 = new PrintStream("turbulences.PPM");
		ps2.println("P3");
		ps2.println(flameaccuTurb.width() + " " + flameaccuTurb.height());
		ps2.println("100");
		int h2 = flameaccuTurb.height();
		int w2 = flameaccuTurb.width();
		System.out.println("Création de l'image Turbulences...");
		for (int i = 0; i < h2; i++) {
			for (int j = 0; j < w2; j++) {
				ps2.print(((int) (100*(flameaccuTurb.intensity(j, i)))) + " ");
			}
			ps2.print("\n");
		}
		ps2.flush();
